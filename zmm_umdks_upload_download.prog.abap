...
* Regel IDs angewandt: #105 #111 #116 #142
************************************************************************

REPORT ZMUEB_STUECKELUNG_SERVER_DKS .

parameters:
p_ifilc1 LIKE rlgrap-filename default 'C:\Uml_DKS.CSV',
p_ofilc1 LIKE rlgrap-filename
  default '/usr/sap/module/mm/input/DKS_UMLAG'.
...
*$smart (F) 2010-08-02 - #111 DATASET statement (A)
  ...
  loop at table1.
    check sy-tabix > 1.
    split table1-record at ';'
    into matrec-artnr
    matrec-charge
    matrec-menge
    matrec-datum
    matrec-lgnr IN CHARACTER MODE .              "smart: 2010-08-02 #116
    check matrec-artnr <> space.
    condense matrec-menge.
    condense matrec-artnr.
    replace ',' with '.' into matrec-menge.
